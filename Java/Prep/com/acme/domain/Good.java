package com.acme.domain;

import java.util.*;

public abstract class Good implements Product, Comparable<Good>{

    public enum UnitOfMeasure{LITER, GALLON, CUBIC_METER, CUBIC_FEET};
    private String name;
    private int modelNumber;
    private double height;
    private UnitOfMeasure unitOfMeasure;
    private boolean flammable;
    private double weightPerUofM;

    private static List<Good> catlog;

    public Good(){}


    public Good(String name, int modelNumber, double height, UnitOfMeasure unitOfMeasure, boolean flammable, double weightPerUofM) {
        this.name = name;
        this.modelNumber = modelNumber;
        this.height = height;
        this.unitOfMeasure = unitOfMeasure;
        this.flammable = flammable;
        this.weightPerUofM = weightPerUofM;
    }


    static{
        catlog = new ArrayList<>();
        Liquid glue = new Liquid("Acme Glue", 2334, 4,
                UnitOfMeasure.LITER, false, 15, 6);
        Liquid paint = new Liquid("Acme Invisible Paint", 2490, 0.65,
                UnitOfMeasure.GALLON, true, 0.70, 12);
        Solid anvil = new Solid("Acme Anvil", 1668, 0.3,
                UnitOfMeasure.CUBIC_METER, false, 500, 0.25, 0.3);
        Solid safe = new Solid("Acme Safe", 1672, 1.0,
                UnitOfMeasure.CUBIC_METER, false, 300, 0.5, 0.5);
        Solid balloon = new Solid("Acme Balloon", 1401, 15,
                UnitOfMeasure.CUBIC_FEET, false, 10, 5, 5);
        Solid pistol = new Solid("Acme Disintegrating Pistol", 1587,
                0.1, UnitOfMeasure.CUBIC_FEET, false, 1, 0.5, 2);
        Liquid nitro = new Liquid("Acme Nitroglycerin", 4289, 1.0,
                UnitOfMeasure.CUBIC_METER, true, 0.25, 1.5);
        Liquid oil = new Liquid("Acme Oil", 4275, 1.0,
                UnitOfMeasure.CUBIC_METER, true, 0.25, 1.5);

        catlog.add(glue);
        catlog.add(paint);
        catlog.add(anvil);
        catlog.add(safe);
        catlog.add(balloon);
        catlog.add(pistol);
        catlog.add(nitro);
        catlog.add(oil);

    }



    public String getName() {
        return name;
    }

    public void setName(String name) {
        this.name = name;
    }

    public int getModelNumber() {
        return modelNumber;
    }

    public void setModelNumber(int modelNumber) {
        this.modelNumber = modelNumber;
    }

    public double getHeight() {
        return height;
    }

    public void setHeight(double height) {
        this.height = height;
    }

    public UnitOfMeasure getUnitOfMeasure() {
        return unitOfMeasure;
    }

    public void setUnitOfMeasure(UnitOfMeasure unitOfMeasure) {
        this.unitOfMeasure = unitOfMeasure;
    }

    public boolean isFlammable() {
        return flammable;
    }

    public void setFlammable(boolean flammable) {
        this.flammable = flammable;
    }

    public double getWeightPerUofM() {
        return weightPerUofM;
    }

    public void setWeightPerUofM(double weightPerUofM) {
        this.weightPerUofM = weightPerUofM;
    }

    public abstract double volume();

    public double weight(){
        return volume() * weightPerUofM;
    }

    public static List<Good> getCatalog(){
        Collections.sort(catlog);
        return catlog;
    }

    public static Set<Good> getFlammables(){
        Iterator<Good> itr = Good.getCatalog().iterator();
        Set<Good> flammables = new HashSet<>();
        while(itr.hasNext()){
            Good curr = (Good) itr.next();
            if(curr.isFlammable()){
                flammables.add(curr);
            }
        }
        return flammables;
    }


    @Override
    public String toString(){
        return name + " - " + modelNumber;
    }

    @Override
    public int compareTo(Good g){
        return getName().compareTo(g.getName());
    }
}
